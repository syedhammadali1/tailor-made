        ...
        <div class="container">
            <div class="d-flex align-items-start">
                ...
                <div class="aiz-user-panel">
                              ...
                              <table class="table aiz-table mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{translate('Club Points Type')}}</th>
                                ...
                                <tbody>
                                    @foreach ($club_points as $key => $club_point)
                                        <tr>
                                            ...
                                            <td>
                                                @if ($club_point->pointType != null)
                                                    {{ ucfirst($club_point->pointType->name) }}
                                                @else
                                                    ---
                                                @endif
                                            </td>
                                            ...
                                            <td class="text-right">
                                                @if ($club_point->convert_status == 0)
                                                    <button onclick="convert_point({{ $club_point->id }})" class="btn btn-sm btn-styled btn-primary convertPId-{{$club_point->id}}">{{translate('Convert Now')}}</button>
                                                @else
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h5 class="mb-0 h6">{{ translate('Convert Time To Points')}}</h5>
                        </div>
                          <div class="card-body">
                              <table class="table aiz-table mb-0">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th>{{translate('Time In Minutes')}}</th>
                                ...
                                <tbody>
                                        ...
                                        <tr>
                                            <td>#</td>
                                            <td>
                                                {{ (int) auth()->user()->spended_minutes_on_site}}
                                            </td>
                                            <td>
                                               {{  (int) (auth()->user()->spended_minutes_on_site / 10 )}}
                                            </td>

                                            <td class="text-right">
                                            <form action="{{ route('convert_time_to_points') }}">
                                                <button type="submit" class="btn btn-sm btn-styled btn-primary">{{translate('Convert Now')}}</button>
                                            </form>
                                            </td>
...
@section('script')
    <script type="text/javascript">
        ...
        {
            $('.convertPId-'+el).attr("disabled", true);
