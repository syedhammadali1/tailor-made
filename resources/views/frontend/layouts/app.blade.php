<!DOCTYPE html>
@isset(\App\Models\Language::where('app_lang_code', Session::get('locale', Config::get('app.locale')))->first()->rtl)
@if(\App\Models\Language::where('app_lang_code', Session::get('locale', Config::get('app.locale')))->first()->rtl == 1)
...
<head>
    ...
    <link rel="stylesheet" href="{{ static_asset('assets/css/lightbox.min.css') }}">
...
<body>
    ...
    <script src="{{ static_asset('assets/js/lightbox.min.js') }}"></script>
    <script>
                ...
                $.ajax({
                   ...
                   success: function(data){
                        ...
                        if(parseInt(data.in_stock) == 0 && data.digital  == 0){
                           $('.request-to-personalise').addClass('d-none');
                           $('.buy-now').addClass('d-none');
                           $('.add-to-cart').addClass('d-none');
                           $('.out-of-stock').removeClass('d-none');
                        }
                        else{
                            $('.request-to-personalise').removeClass('d-none');
        ...
        function requestForSelectMeasurment(){
            if('{{ \Auth::check() }}' !== '1')
                return $('#login_modal').modal();

            if(checkAddToCartValidity()) {
                // $('#request-to-personaliser').modal()
                $('#select-to-measurment').modal()
            }
            else{
                AIZ.plugins.notify('warning', "{{ translate('Please choose all the options') }}");
            }
        ...
        function requestToPersonliserSubmit(e){
            // e.preventDefault();
            // alert('ssss')

            var requestData = $('#option-choice-form').serializeArray().concat($('.request-to-personliser-form').serializeArray())
            ...
            if(checkAddToCartValidity()) {
                ...
                $.ajax({
                    type:"POST",
                    url: '{{ route('request.add_request_to_personaliser') }}',
                    data: fd,
                    headers: {
                    ...
                    success: function(data){
                        ...
                        $('#request-to-personaliser').modal('toggle')
                        alert(data.msg)
                    },
                    error: function() {
...
$(document).on('click', '.f-m-btn-nearby', function(){
   ...
   var url = "{{ route('requests.nearby_measurers_customers', ":id") }}";
...
$('.request-to-personliser-form').submit(function (e) {
    ...
    $('#option-choice-form').serializeArray().forEach(function(field) {
            ...
            if(checkAddToCartValidity()) {
                ...
                $.ajax({
                    ...
                    success: function(data){
                        // console.log(data.msg);
                        $('#request-to-personaliser').modal('toggle')
                        $('#request-for-product').modal('toggle')
                        alert(data.msg)
                    },
                    error: function() {
                        alert('Something went wrong, try again later !')
...
@include('scripts.global-script')

<script src="{{ asset('/public/assets/js/jquery.datetimepicker.js') }}"></script>
    @yield('script')
