...
<a href="javascript:void(0)" class="d-flex align-items-center text-reset h-100" data-toggle="dropdown"
    ...
    <span class="flex-grow-1 ml-1">
        ...
        @endif
        <span class="nav-box-text d-none d-xl-block opacity-70">{{  GoogleTranslate::trans('Cart',session()->get('localelang')) }}</span>
    </span>
</a>
<div class="dropdown-menu dropdown-menu-right dropdown-menu-lg p-0 stop-propagation">
    ...
    @if (isset($cart) && count($cart) > 0)
        <div class="p-3 fs-15 fw-600 p-3 border-bottom">
            {{  GoogleTranslate::trans('Cart Items',session()->get('localelang')) }}
        </div>
        ...
        <div class="px-3 py-2 fs-15 border-top d-flex justify-content-between">
            <span class="opacity-60">{{  GoogleTranslate::trans('Subtotal',session()->get('localelang')) }}</span>
            ...
            <ul class="list-inline mb-0">
                <li class="list-inline-item">
                    <a href="{{ route('cart') }}" class="btn btn-soft-primary btn-sm">
                        {{  GoogleTranslate::trans('View cart',session()->get('localelang')) }}
                    </a>
                </li>
                @if (Auth::check())
                    <li class="list-inline-item">
                        <a href="{{ route('checkout.shipping_info') }}" class="btn btn-primary btn-sm">
                            {{  GoogleTranslate::trans('Checkout',session()->get('localelang')) }}
                        </a>
                    </li>
                @endif
            </ul>
    ...
    @else
        <div class="text-center p-3">
            ...
            <h3 class="h6 fw-700">{{  GoogleTranslate::trans('Your Cart is empty',session()->get('localelang')) }}</h3>
        </div>
    @endif
