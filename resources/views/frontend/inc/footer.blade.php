        ...
        <div class="row no-gutters">
            <div class="col-lg-3 col-md-6">
                <a class="text-reset border-left text-center p-4 d-block" href="{{ route('terms') }}">
                    ...
                    <h4 class="h6">{{  GoogleTranslate::trans('Terms & conditions',session()->get('localelang')) }}</h4>
                </a>
            </div>
            <div class="col-lg-3 col-md-6">
                <a class="text-reset border-left text-center p-4 d-block" href="{{ route('returnpolicy') }}">
                    <i class="la la-mail-reply la-3x text-primary mb-2"></i>
                    <h4 class="h6">{{  GoogleTranslate::trans('Return Policy',session()->get('localelang')) }}</h4>
                </a>
            </div>
            <div class="col-lg-3 col-md-6">
                <a class="text-reset border-left text-center p-4 d-block" href="{{ route('supportpolicy') }}">
                    <i class="la la-support la-3x text-primary mb-2"></i>
                    <h4 class="h6">{{  GoogleTranslate::trans('Support Policy',session()->get('localelang')) }}</h4>
                </a>
            </div>
            <div class="col-lg-3 col-md-6">
                <a class="text-reset border-left border-right text-center p-4 d-block" href="{{ route('privacypolicy') }}">
                    <i class="las la-exclamation-circle la-3x text-primary mb-2"></i>
                    <h4 class="h6">{{  GoogleTranslate::trans('Privacy Policy',session()->get('localelang')) }}</h4>
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="container">
        <div class="row">
                    ...
                    <div class="d-inline-block d-md-block mb-4">
                        <form class="form-inline" method="POST" action="{{ route('subscribers.store') }}">
                            ...
                            <div class="form-group mb-0">
                                <input type="email" class="form-control" placeholder="{{  GoogleTranslate::trans('Your Email Address',session()->get('localelang')) }}" name="email" required>
                            </div>
                            <button type="submit" class="btn btn-primary">
                                {{  GoogleTranslate::trans('Subscribe',session()->get('localelang')) }}
                            </button>
                        </form>
                    </div>
            ...
            <div class="col-lg-3 ml-xl-auto col-md-4 mr-0">
                <div class="text-center text-md-left mt-4">
                    <h4 class="fs-13 text-uppercase fw-600 border-bottom border-gray-900 pb-2 mb-4">
                        {{  GoogleTranslate::trans('Contact Info',session()->get('localelang')) }}
                    </h4>
                    <ul class="list-unstyled">
                        <li class="mb-2">
                           <span class="d-block opacity-30">{{  GoogleTranslate::trans('Address',session()->get('localelang')) }}:</span>
                           <span class="d-block opacity-70">{{ get_setting('contact_address',null,App::getLocale()) }}</span>
                        </li>
                        <li class="mb-2">
                           <span class="d-block opacity-30">{{ GoogleTranslate::trans('Phone',session()->get('localelang'))}}:</span>
                           <span class="d-block opacity-70">{{ get_setting('contact_phone') }}</span>
                        </li>
                        <li class="mb-2">
                           <span class="d-block opacity-30">{{ GoogleTranslate::trans('Email',session()->get('localelang'))}}:</span>
            ...
            <div class="col-md-4 col-lg-2">
                <div class="text-center text-md-left mt-4">
                    <h4 class="fs-13 text-uppercase fw-600 border-bottom border-gray-900 pb-2 mb-4">
                        {{  GoogleTranslate::trans('My Account',session()->get('localelang')) }}
                    </h4>
                    <ul class="list-unstyled">
                        @if (Auth::check())
                            <li class="mb-2">
                                <a class="opacity-50 hov-opacity-100 text-reset" href="{{ route('logout') }}">
                                    {{  GoogleTranslate::trans('Logout',session()->get('localelang')) }}
                                </a>
                            </li>
                        @else
                            <li class="mb-2">
                                <a class="opacity-50 hov-opacity-100 text-reset" href="{{ route('user.login') }}">
                                    {{  GoogleTranslate::trans('Login',session()->get('localelang')) }}
                                </a>
                            </li>
                        @endif
                        <li class="mb-2">
                            <a class="opacity-50 hov-opacity-100 text-reset" href="{{ route('purchase_history.index') }}">
                                {{  GoogleTranslate::trans('Order History',session()->get('localelang')) }}
                            </a>
                        </li>
                        <li class="mb-2">
                            <a class="opacity-50 hov-opacity-100 text-reset" href="{{ route('wishlists.index') }}">
                                {{  GoogleTranslate::trans('My Wishlist',session()->get('localelang')) }}
                            </a>
                        </li>
                        <li class="mb-2">
                            <a class="opacity-50 hov-opacity-100 text-reset" href="{{ route('orders.track') }}">
                                {{  GoogleTranslate::trans('Track Order',session()->get('localelang')) }}
                            </a>
                        </li>
                        @if (addon_is_activated('affiliate_system'))
                            <li class="mb-2">
                                <a class="opacity-50 hov-opacity-100 text-light" href="{{ route('affiliate.apply') }}">{{  GoogleTranslate::trans('Be an affiliate partner',session()->get('localelang'))}}</a>
                            </li>
                        @endif
                    </ul>
                </div>
                @if (get_setting('vendor_system_activation') == 1)
                    <div class="text-center text-md-left mt-4">
                        <h4 class="fs-13 text-uppercase fw-600 border-bottom border-gray-900 pb-2 mb-4">
                            {{  GoogleTranslate::trans('Be a Seller',session()->get('localelang')) }}
                        </h4>
                        <a href="{{ route('shops.create') }}" class="btn btn-primary btn-sm shadow-md">
                            {{  GoogleTranslate::trans('Apply Now',session()->get('localelang')) }}
                        </a>
                    </div>
                    <div class="text-center text-md-left mt-4">
                        <h4 class="fs-13 text-uppercase fw-600 border-bottom border-gray-900 pb-2 mb-4">
                            {{  GoogleTranslate::trans('Be a Measurer',session()->get('localelang')) }}
                        </h4>
                        <a href="{{ route('measurer.register-measurer') }}" class="btn btn-primary btn-sm shadow-md">
                            {{  GoogleTranslate::trans('Apply Now',session()->get('localelang')) }}
                        </a>
                    </div>
                @endif
    ...
    <div class="row align-items-center gutters-5">
        <div class="col">
            <a href="{{ route('home') }}" class="text-reset d-block text-center pb-2 pt-3">
                ...
                <span class="d-block fs-10 fw-600 opacity-60 {{ areActiveRoutes(['home'],'opacity-100 fw-600')}}">{{  GoogleTranslate::trans('Home',session()->get('localelang')) }}</span>
            </a>
        </div>
        <div class="col">
            <a href="{{ route('categories.all') }}" class="text-reset d-block text-center pb-2 pt-3">
                ...
                <span class="d-block fs-10 fw-600 opacity-60 {{ areActiveRoutes(['categories.all'],'opacity-100 fw-600')}}">{{  GoogleTranslate::trans('Categories',session()->get('localelang')) }}</span>
            </a>
        </div>
        @php
            if(auth()->user() != null) {
        ...
        <div class="col-auto">
            <a href="{{ route('cart') }}" class="text-reset d-block text-center pb-2 pt-3">
                ...
                <span class="d-block mt-1 fs-10 fw-600 opacity-60 {{ areActiveRoutes(['cart'],'opacity-100 fw-600')}}">
                    {{  GoogleTranslate::trans('Cart',session()->get('localelang')) }}
                    @php
        ...
        <div class="col">
            <a href="{{ route('all-notifications') }}" class="text-reset d-block text-center pb-2 pt-3">
                ...
                <span class="d-block fs-10 fw-600 opacity-60 {{ areActiveRoutes(['all-notifications'],'opacity-100 fw-600')}}">{{  GoogleTranslate::trans('Notifications',session()->get('localelang')) }}</span>
            </a>
        </div>
        <div class="col">
        @if (Auth::check())
            @if(isAdmin())
                <a href="{{ route('admin.dashboard') }}" class="text-reset d-block text-center pb-2 pt-3">
                    ...
                    <span class="d-block fs-10 fw-600 opacity-60">{{  GoogleTranslate::trans('Account',session()->get('localelang')) }}</span>
                </a>
            @else
