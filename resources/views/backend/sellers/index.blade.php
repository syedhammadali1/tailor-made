            ...
            <table class="table aiz-table mb-0">
                ...
                @foreach($shops as $key => $shop)
                    <tr>
                        ...
                        <td>
                            @if ($shop->user->is_approved_by_admin == 0)
                                   <p> Pending </p>
                            @elseif($shop->user->is_approved_by_admin == 1)
                                <p> Approved</p>
                            @elseif($shop->user->is_approved_by_admin == 2)
                            <p> Rejected</p>
                            @endif
                        </td>
                        <td>
                            @if ($shop->user->is_approved_by_admin == 0)
                            <div class="d-flex">
                                <a href="{{route('approvedByAdmin',['user_id' => $shop->user->id,'Approved' => true])}}" class="btn btn-soft-dark btn-sm d-flex align-items-center rounded-circle approved-btn">
                                    <i class="las la-check"></i>
                                </a>
                                <a href="{{route('approvedByAdmin',['user_id' => $shop->user->id,'Approved' => false])}}"  class="btn btn-soft-danger btn-sm d-flex align-items-center rounded-circle reject-btn">
...
@section('script')
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            // $(document).on('click','.approved-btn , .reject-btn',function (e) {
            //     e.preventDefault();
            //     let user_id = $(this).data('user-id');
            //     let Approved = $(this).hasClass('approved-btn')
            //    console.log( );
            //     $.ajax({
            //         type: "GET",
            //         url: "{{route('approvedByAdmin')}}",
            //         data: {
            //             'user_id':user_id,
            //             'Approved':Approved
            //         },
