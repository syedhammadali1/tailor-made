...
<form class="" action="{{route('seller.products.update', $product->id)}}" method="POST" enctype="multipart/form-data"
        ...
        <div class="col-lg-8">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        <label class="col-md-3 col-from-label">{{translate('Is Personalise Product')}} </label>
                        <div class="col-md-8">

                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input type="checkbox" name="is_personalise" id="is_personalise" value="1" {{ $product->is_personalise == 1 ? 'checked' : '' }} >
                                <span></span>
                            </label>
                    ...
                    @if ($product->is_personalise == 1)
                    <div class="form-group row" id="personalize_product_type_id">
                        <label class="col-md-3 col-from-label">{{translate('Personalise Product Type')}}</label>
                        <div class="col-md-8">
                            <select class="form-control aiz-selectpicker" name="personalize_product_type_id" id="personalize_product_type_id"
                                 required>
                                @foreach ($personalise_product_types as $item)
                                <option value="{{ $item->id }}" @if($product->personalize_product_type_id == $item->id) selected
    ...
    <div class="col-lg-4">
        ...
        @isset(auth()->user()->userProfileById->profileName)
            @if (in_array('Add campaign profile',auth()->user()->userProfileById->profileName->pluck('profile_name')->toArray()))
            <div class="card">
            <div class="card-header">
                <h5 class="mb-0 h6">{{translate('Crazy Sunday')}}</h5>
            </div>
            <div class="card-body">
                    <div class="form-group row">
                        <div class="col-md-12">
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    <label class="aiz-switch aiz-switch-success mb-0">
                                        <input type="checkbox" name="is_crazy_sunday" value="1"
                                               @if($product->is_crazy_sunday == 1) checked @endif>
                                        <span></span>
                                    </label>
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    $("[name=is_personalise]").on("change", function (){

        if (this.checked) {
            $("#personalize_product_type_id").css({display: "block"});
        }

        else{
            $("#personalize_product_type_id").css({display: "none"});
        }
        });
