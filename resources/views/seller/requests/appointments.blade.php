                ...
                <table class="table aiz-table mb-0">
                    <thead>
                        <tr>
                            ...
                            <th data-breakpoints="md">{{ translate('Request')}}</th>
                            <th data-breakpoints="lg">{{ translate('Commission Price')}}</th>
                            <th data-breakpoints="lg">{{ translate('Status')}}</th>
                            {{-- <th class="text-right">{{ translate('Options')}}</th> --}}
                            <th>{{ translate('Appointment Date Time')}}</th>
                            @isset($completedMeasures)
                            <th class="text-left">{{ translate('See measurments')}}</th>
                            @else
                        ...
                        @foreach ($appointments as $key => $appointment)
                            ...
                            @if($appointment != null)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('seller.requests.index').'?request_id='.$appointment->request->id }}">View Request</a>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        {{-- {{  $appointment->appointment_status == 1 ? 'Approved' : 'Pending'  }} --}}
                                    ...
                                    @else
                                        <td>
                                            {{-- @if( date('Y-m-d' , strtotime($appointment->datetime)) < $today)
                                                <p>Expired</p>
                                            @elseif( date('Y-m-d' , strtotime($appointment->datetime)) > $today)
                                                <p>Up Coming</p>
                                            @elseif( date('Y-m-d' , strtotime($appointment->datetime)) == $today)
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                        @if($appointment->appointment_status == 0)
                                                            ...
                                                            <a href="javascript:;" class="dropdown-item  m-a-accept-btn">Accept</a>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                    ...
                                                    <a onclick="actionConfirmCheck('{{ route('measurer-appointment.accept-reject', ['id' => encrypt(@$appointment->id), 'status' => encrypt('accept')] ) }}', 'Are you sure you want accept this appointment')" href="javascript:;" class="dropdown-item  m-a-accept-btn">Accept</a>
                                                        {{-- <a href="javascript:;" class="dropdown-item  m-a-accept-btn">Accept {{ $appointment->appointment_status }}</a> --}}
                                                        <a onclick="actionConfirmCheck('{{ route('measurer-appointment.accept-reject', ['id' => encrypt(@$appointment->id), 'status' => encrypt('reject')] ) }}', 'Are you sure you want reject this appointment')" href="javascript:;" class="dropdown-item" >Reject</a>
                                    ...
                                    <td>
                                        {{-- <button type="button" data-toggle="modal" data-target="#find-measurer-modal"  data-request-id="{{$request->id}}" class="f-m-btn btn btn-soft-primary mr-2 find-measurer-btn fw-600" >
