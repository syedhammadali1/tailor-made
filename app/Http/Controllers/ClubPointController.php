...
class ClubPointController extends Controller
{
    ...
    {
        $club_points = ClubPoint::with('pointType')->where('user_id', Auth::user()->id)->latest()->paginate(15);
    ...
    public function convert_time_to_points()
    {

        $user = User::find(auth()->id());

        //convert minutes to points
        $points = (int) ($user->spended_minutes_on_site / 10);

        // insert point to clubpoint table
        $club_points = new ClubPoint;
        $club_points->user_id = $user->id;
        $club_points->points = $points;
        $club_points->club_point_type_id = 4;
        $club_points->user_id = $user->id;
        $club_points->convert_status = 0;
        $club_points->save();

        // empty spended_minutes_on_site column
        $user->spended_minutes_on_site = 0.00;
        $user->save();
