...
if (!function_exists('filter_products')) {
    function filter_products($products)
    {
        ...
        $ip = '50.174.42.230'; // static IP address get

        $data = Location::get($ip);
        $customer_country = $data->countryName;



        // this query will get all sellers those have classic profile and
        //  those doesn't belong to customer country
         $classic_profile_seller = User::whereIn('id', $verified_sellers)
         ->whereRelation('userProfileById', 'profile_id', 1)
         ->where('country','!=',$customer_country)
         ->pluck('id')
         ->toArray();

         $final_verified_sellers = array_diff($verified_sellers, $classic_profile_seller);

        $verified_sellers = $final_verified_sellers;
...
function translate($key, $lang = null, $addslashes = false)
{
    ...
    return GoogleTranslate::trans($key,session()->get('localelang'));
    ...
    dd(addslashes(trim($translations_en[$lang_key])));
...
if (!function_exists('isProductInCampaign')) {
    function isProductInCampaign($prodID)
    {
        $flash_deal = FlashDeal::with(['flash_deal_products'])->where('status', 1)->where('featured', 1)->first();
        if ($flash_deal){
           $dealsProducts = $flash_deal->flash_deal_products->whereIn('product_id',$prodID);
            if (count($dealsProducts) == count($prodID)){
                return true;
            }
        }
        return false;
...
if (!function_exists('appointment_stutus')) {
    function appointment_stutus($status)
    {

        if(is_numeric($status) && $status <= 7) {
            $statuses = ['Pending', 'Confirmed', 'Picked Up', 'On The Way', 'Cancel', 'Reject','Completed','Expired'];
            return $statuses[$status];
        }
        return false;
    }
}
if (!function_exists('request_to_be_model_stutus')) {
    function request_to_be_model_stutus($status)
    {

        if(is_numeric($status) && $status <= 4) {
            $statuses = ['Pending', 'Confirmed', 'Reject','Completed'];
            return $statuses[$status];
        }
        return false;
    }
...
if (!function_exists('remove_image')) {
    ...
    {
        if(File::exists(public_path('assets/img/'.$image))){
            File::delete(public_path('assets/img/'.$image));
        }
    }
}

if (!function_exists('appointments_expire')) {
     function appointments_expire()
    {
        // get all pending appointment request
        $appointments = RequestAppointment::where('appointment_status', [0])->get();

        foreach ($appointments as $key => $appointment) {
            $today = new DateTime();
            $expireDateTime = new DateTime($appointment->created_at->modify('+1 day') );
